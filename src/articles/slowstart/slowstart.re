...
//image[cwnd][cwnd][scale=0.4]
//image[sake][飲酒量][scale=0.4]
//}
...
//}
//chat[花名]{
TCP @<ruby>{inca,陰キャ}st……？
人の痛みがわかる子になりなさい…みたいな名前だね……。
//}
...
//texequation{
\mathrm{W}(t) = C(t-\sqrt[3]{\frac{W_{max}\beta}{C}})^3 + W_{max}
//}
...
5-tupleか何かのハッシュによって各フローで一定の経路にするなら大丈夫そう@<fn>{timely}、パケットごとに分散させるなんて輻輳制御どころかパケットリオーダリングでパフォーマンスが低下する懸念@<fn>{kenen}@<fn>{dixit13}から通常はやらないので問題ないのかも知れません。
...
//footnote[timely][論文中でもClosネットワークで評価していました。ECMPの話は出ていませんでしたが、おそらく5-tupleで振り分けたのでしょう。]
//footnote[kenen][とはいえ、実はパケットごとに負荷分散（Packet Spraying）によってパケットリオーダリングが発生してもパフォーマンスへの影響はさほど気にしなくても良い、むしろ分散されることのメリットがあるという文献もあります。（RTTを重視するTIMELYでも本当に同じことが言えるかは分かりませんが……。）]
...
//image[cell][青海展示棟におけるLTE基地局の探索結果][scale=0.9]
