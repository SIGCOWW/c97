= 【スロウスタート】花名「レイヤ4輻輳制御？」
@<author>{おたく風邪に罹患した, lrks}
//lead{
読者がじゃぶじゃぶ読みたくなるような射幸心を煽りまくる説明文章
//}
...
確か、ACKを待たずに送れるデータ量@<fn>{in-flight}のことだったと思うけど……。
//}
//rchat[たまて]{
...
ちなみにCCPですが、概要図は図～のとおりで、``Datapath''と``Algorithms''が分離されています。
...
CUBICは、cwndをもっと早く上げたいということで提案されたBIC@<fn>{xu04}のcwndの変化（図〜）を、次の3次関数@<fn>{cubic}で近似（図〜）したアルゴリズムです。
//texequation{
...
//footnote[cardwell17][N.Cardwell, et al.，``BBR: Congestion-Based Congestion Control''，ACM Queue，2017．]
...
図～がcwndに対するレイテンシと転送レートの関係で、レイテンシの観点からはキューイングされなければ最小値@<b>{RTprop}@<fn>{rtprop}を維持できる、転送レートの観点からはどんどん送れば最大値@<b>{BtlBw}@<fn>{btlbw}を達成できることになります。
ちょうどBDP@<fn>{bdp}のところだと都合が良さそう、ということでBBRではRTTと転送レートを観測@<fn>{delay-based}@<fn>{vegas}しRTpropとBltBwを推定してcwndをBDPと等しくなるように調整しています。
// 図 ... BBRより引用。縦軸は上図と下図で独立している。横軸はcwnd(ニアリーAmount In-flight)
...
図～のように、あるパケットがスイッチを通った際にスイッチがパケット内にINTを書いてくれるので、HPCCではこれをNICから利用します。
//footnote[int][In-band Network Telemetryとも呼ばれるようです。]

……いや、そんなINTを読み書きするスイッチやNICがあるのでしょうか？
自分たちで作ればあります、ということでHPCCの著者はP4@<fn>{p4}でスイッチを作り、FPGAでNICを作って検証、高いパフォーマンスを実現できたそうです。
...
//footnote[omoshiroi][ある場所で検証した際は、特殊な事業者のMNC (Mobile Network Code)が見えて面白かったです。]
