...
//profile{
本当は「スマタリングTCP/IP ～Promiscuous編～」というコピ本を発行して、「Iwaraのクローリング ～○○○○○じゃないと抜けない～」「○○○○○のコラ自動作成～こんな風に→されちゃうお話～」をやりたかった。次号にご期待ください。
//}
//lead{
本稿ではレイヤ4輻輳制御の概要といくつかの輻輳制御アルゴリズム、具体的にはReno、CUBIC、BBR、Copa、DCTCP、HPCCを「スロウスタート」のキャラクターに述べてもらいます。
//}
...
//chat[たまて]{
ギガはマシマシでした！
これはもしかすると、@<b>{レイヤ4輻輳制御}が原因かも@<fn>{maji}ですね〜
//}
//rchat[花名]{
レイヤ4輻輳制御？
//}
//footnote[maji][マジレスすると、この時点で輻輳制御の影響を疑うのは早計、そして輻輳制御はおそらく関係ないと思います。ただ本稿は「テーマがさき　背景はあと」です。]

花名ちゃんが気になっているので、本稿では「レイヤ4輻輳制御」について述べていきます。
さて、レイヤ4輻輳制御？とは何でしょう？
@<b>{レイヤ4}で@<b>{輻輳}を制御するみたいですが？

レイヤ4は、OSIモデルにおけるトランスポート層のこと、つまりTCP@<fn>{tcp}@<fn>{pacing}やUDPなどのことです。
輻輳とは？英語だと``Congestion''、また広辞苑によると「@<ruby>{輻,や}@<fn>{spoke}が@<ruby>{轂,こしき}にあつまる意。方々から集まること。物が1カ所にこみあうこと。」だそうです@<fn>{soudayo}。
...
SYN Floodでサーバのコネクションやファイルディスクリプタを食いつぶす、またはサーバに負荷をかけるようなリクエストを送ってリソースを食いつぶし、新しいコネクションを受け入れられないようにしても輻輳です。
...
//footnote[hub][車輪中央のハブのことだそうです。用例:「輻W L2スイッチは全ポートミラーに設定してもL1の轂でわない」]
...
上記のように書くと、輻輳制御は非常に特殊な状況でのみ発動するように感じられるかも知れません。
...
ここでいう@<bou>{周り}には、自分が生み出した他のフローも含まれるため、結局は自分も損することになります。
輻輳制御は必要不可欠です。
...
輻輳制御入門こと「TCP技術入門」@<fn>{annaga19}によれば、「受信側が許容可能なデータ量を送信側に通知しながら送信データ量を調整する」ことがフロー制御だと書かれてます。一方で、輻輳制御ではネットワーク全体のことを考えて、輻輳しないよう制御する旨も書かれています。
...
//footnote[alizadeh13][M.Alizadeh, et al.，``pFabric: Minimal near-optimal datacenter transport''，ACM SIGCOMM，2013．]
...
しかし、輻輳制御としつつも``Receiver-side congestion control''と述べた文献@<fn>{kumar19}があったり、詳細は後述しますがINT (In‐band Network Telemetry)で明示的な情報を取得して制御したりするアルゴリズムがあります。
//footnote[layer3][IPヘッダのECNを利用するアルゴリズムのことです。]
//footnote[call][例えば災害時に通話を制限する「発呼制限」は輻輳制御にあたると思います。]
//footnote[kumar19][P.Kumar, et al.，``PicNIC: Predictable Virtualized NIC''，ACM SIGCOMM，2019．]
...
確か、ACKを待たずに送れるデータ量@<fn>{inflight}のことだったと思うけど……。
//}
//rchat[たまて]{
確かに「ウィンドウサイズ」はその通りですね。
ただ、ここではTCPヘッダの中の「ウィンドウサイズ」は、受信ウィンドウ（rwnd: Receive Window）のこととします。自分が受信できるバイト数@<fn>{octet}を相手に通知するわけですね。
相手、すなわちデータ送信側では、cwndとrwndのうち小さいほうを本当の「ウィンドウサイズ」として扱う@<fn>{wnd}んですよ！
//}
//footnote[octet][厳密には「オクテット」になります。]
//footnote[inflight][対応するACKが返っていないデータは@<b>{In-flight}と表現されることがあります。]
//footnote[wnd][すべてのTCP/IPプロトコルスタックの実装がそうなっているかは分かりませんが、少なくともRFC 5681ではそう定義されています。]
以上の会話を盗み聞きした結果、rwndは相手から明示的に与えられることが分かりました。
では、cwndはどう知るのでしょうか……？初期値くらいは決められそうですが、どれぐらい増減させるかは分からないのでは…？

@<bou>{おおむね}cwndの値は、探索によって推定します。
データ送信側で、これ以上送ったら輻輳するな…と思ったら減らす、まだ送れそうだなと思ったら増やすわけです。
...
具体的にはNewReno、CUBIC、BBR、Copaなどのことですね。
また、条件を限定すれば正確な推定ができるようになってパフォーマンスが上がるのでは？ということで、特定用途向けのアルゴリズムもあります。
モバイルネットワークを想定したSprout、DC内のネットワークを想定したDCTCPや、その中でもRoCE v2を想定したDCQCNやHPCCのことです。
一部は出典とともに詳細を後述します。
...
それどころか、ネットワーク機器がトランスポート層を読み書きする@<fn>{napt}なんて……。
OSI7階層、レイヤとは一体……？
//}
//footnote[napt][NAPT機器もこの一種です。TCPペイロードまで改変するMiddleboxもあります。]

ところで、cwndの推定はどのタイミングで行うのでしょう？
最低限、パケットを送る前に推定が終わっていれば良さそうです。
ただ、まさにパケットを送る瞬間に推定するのは余計な時間がかかるため、ACKが返ってきたときやパケロスなど重要なイベントの発生を契機にします。
...
//image[ccp][CCPの概要図（論文から引用）][scale=0.5]

ここで``Datapath''とは、Linuxカーネル@<fn>{ccp-kernel}やmTCP、QUICのこと、``Algorithms''はユーザランドで実装された輻輳制御アルゴリズムのことを指します。
こうすることで、アルゴリズムが30億のデバイスで動きそうな``Write-Once, Run-Anywhere''になり、データパスの制約によらず複雑なアルゴリズムが実現可能です。
さらに、アルゴリズムをマシンの外に出すこともできるので、複数マシンが協調して動くような輻輳制御を実現できるかもといった旨も述べられています。
自律制御から集中制御……OpenFlowや冒頭で述べたpFabricを彷彿とさせますね……。
夢が広がります。
少し前に述べたCopaやNimbus@<fn>{goyal18}など、最近提案されるアルゴリズムはCCPで実装されることがあり、今後もこの傾向が続ければ、新しいアルゴリズムをいち早く試しやすくなるかと思います。
...
//footnote[narayan18][A.Narayan, et al.，``Restructuring Endpoint Congestion Control''，ACM SIGCOMM，2018．]
//footnote[ccp-kernel][正確には@<tt>{ccp-kernel}というカーネルモジュールを介します。]
//footnote[goyal18][P.Goyal, et al.，``Elasticity Detection: A Building Block for Internet Congestion Control''，arXiv:1802.08730，2018．]

//chat[花名]{
これでcwndの推定も済んだし、あとは「ウィンドウサイズ」に達するまでデータを送るだけだね。
心なしか大きい気がするけど、確かに推定時点でのネットワークは輻輳していないから送ってみるね……。
あれ……？輻輳しているような……推定がうまくいかなかったのかな？
//}
//rchat[たまて]{
...
図～にcwnd変化のイメージを、ついでに図～に飲酒で例えたイメージを示します。
次は輻輳制御アルゴリズムの紹介です。

== アルゴリズムのしょうかい
//chat[花名]{
そうなんだ……もっと知りたいな輻輳制御のこと……。
...
マイナーアップデート版の@<b>{NewReno}@<fn>{rfc2582}のことを単に「Reno」と呼ぶこともあります。
//footnote[jacobson88][V.Jacobson, et al.，``Congestion avoidance and control''，ACM SIGCOMM，1988．]
...
//footnote[rfc2582][S.Floyd, et al.，``The NewReno Modification to TCP's Fast Recovery Algorithm''，RFC 2582，1999．]
...
RenoやNewRenoの挙動はシンプルで、図～のようにスロースタート中は指数関数的にcwndを伸ばす、輻輳を検知するとその時点のcwndを半分@<fn>{ssthresh}にして線形に伸ばしていく……ということを繰り返します。
...
=== CUBIC
次は@<b>{CUBIC}@<fn>{ha08}です。
RenoやNewRenoほどではありませんが、Linuxカーネルでは2.6.19（2006年）からデフォルトの輻輳制御アルゴリズムとして用いられている@<fn>{bic}ほか、一部のカーネルのTCP/IPプロトコルスタックを使わないようなアプリケーション@<fn>{bypass}でも用いられ、ほどほどに使われている、また最近の輻輳制御アルゴリズムと比較する際のベースラインにもなっています。
//footnote[ha08][S.Ha, et al.，``CUBIC: A New TCP-Friendly High-Speed TCP Variant''，ACM SIGOPS，2008．]
...
//texequation{
\mathrm{W}(t) = C(t-K)^3 + W_{max}
//}
ここで、@<m>{K = \sqrt[3]{\frac{W_{max}\beta}{C}}}、@<m>{t}は輻輳を検知してからの経過時間、@<m>{W_{max}}は輻輳を検知した時点のcwnd、@<m>{C}はcwndの増加割合を決定するパラメータ、@<m>{\beta}はcwndの減少割合を決定するパラメータです。
//subfig[BICとCUBICにおけるcwndの変化（CUBICの論文より引用）]{
//image[bic][BIC][scale=0.45]
//image[cubic][CUBIC][scale=0.45]
//}

通信中のcwndは、この3次関数が連なるように変化していきます。
実は、以前図～で示したcwnd変化のイメージは、CUBICを思い浮かべながら作図していました。
//footnote[cubic][だから``CUBIC''です。BICとCUBICの著者は3人中2人被っていますが、これが言いたくて``BIC''と名付けたとかないですか？]
//footnote[xu04][L.Xu, et al.，``Binary Increase Congestion Control (BIC) for Fast Long-Distance Networks''，IEEE INFOCOM，2004．]

ところで、3次関数に立方根……？
Linuxカーネルでは浮動小数点数の演算はしない@<fn>{fpu}はずです。
そのため、CUBICの実装では整数演算で頑張ります。
立方根の計算もルックアップテーブルとNewton-Raphson法で行います。
...
=== BBR
そして@<b>{BBR}@<fn>{cardwell16}です。
QUICの輻輳制御アルゴリズムとして用いられている@<fn>{chromium}ほか、Linuxカーネルでも4.9以降であれば利用できることもあり、最近注目されています。
無印の「BBR」（または「BBR v1」）と「BBR v2」に分かれますが、ここでは前者について述べます。
//footnote[cardwell16][N.Cardwell, et al.，``BBR: Congestion-Based Congestion Control''，ACM Queue，2016．]
...
//image[bbr][cwndに対するレイテンシ転送レートの関係（BBRの資料より引用）。横軸はcwndとほぼ同等、縦軸は上図と下図でそれぞれ独立している。][scale=0.5]
...
//footnote[delay-based][RTTを観測するということで@<b>{Delay-Based}と呼ばれることもありますが、違和感ががあるので@<b>{Delay-Sensing}くらいで勘弁してほしいです。QUICの論文（``The QUIC Transport Protocol: Design and Internet-Scale Deployment''）でもそう書かれていますし。]
...
たとえば、レイテンシがさほど低くならない、複雑なネットワークではRTpropとBltBWの推定がうまくいかないのではという懸念、また詳細は後述しますが@<b>{公平性}が不十分なために他のフローから帯域を奪ってスループットを向上しているなど……です@<fn>{dis}。
...
タイマ割り込みをさせる（@<tt>{TSC_DEADLINE}）ためMSRへ書き込む（wrmsr）ことが原因とされています。
そういえば、@<tt>{tcp_bbr.c}には「キューイングにfqを指定しないと高解像度タイマーを利用する」ことが書かれていました。
kvm-clockなど特定のクロックソースが有効か否かにもよりそうですが、少し影響が心配です。
//footnote[pi19][Z.Pi, et al.，``How KVM-based Hybrid Deployment Powers Bytedance’s Biggest Day Ever''，KVM Forum 2019．（@<href>{https://cloud.tencent.com/developer/article/1537481}）]

BBRは、GCPやAWSなど大規模な利用が目立つためかメリットばかりが取り上げられているように見えるのですが、利用は慎重にすべきだと思います。
現在開発中のBBR v2では、先に挙げた課題に対する検討がされているため、これがLinuxカーネルのメインラインに取り込まれるまで待っても良いのではという気がします。
//chat[花名]{
BBRはDelay-Sensingなアルゴリズムで、最近普及してきているけど利用は慎重にかぁ……。
//}

=== Copa
次は@<b>{Copa}@<fn>{arun18}です。
BBRと同様に、スループット向上とレイテンシ削減を同時に達成することを目標にします。
そのため下記のような目的関数を定めて、この値が最大となるような送信レートを最適化によって求め……ってそれは前からあるのでは？
//texequation{
\mathrm{U} = \log{送信レート} - \delta\log{\mathrm{RTT} - \mathrm{RTTprop}}
//}
//footnote[arun18][V.Arun, et al.，``Copa: Practical Delay-Based Congestion Control for the Internet''， 15th USENIX Symposium on Networked Systems Design and Implementation (NSDI 18)，2018．]

ここで注目すべきは@<m>{\delta}です。
これはレイテンシをどれぐらい重視するかの重みで、通常は固定値とします。
しかし、RTTの変動などでLoss-Basedなアルゴリズムと同じリンクを共有していると検知した場合は変動させます。
なぜこのようなことをするのでしょうか？
これは、@<b>{公平性}の配慮を理由に行われます。

あるリンクを複数のフローが共有する場合、そのリンクのリソースは各フローへ均等に割り当てられるべきです。
たとえば、図～のように「インターネット無料」をうたう軽井沢あたりのアパートからインターネットへ4つのフローが存在する場合、ネットワーク機器のリソースは25%ずつ、すなわち帯域幅が25%ずつ、レイテンシは等しくなることが期待されます@<fn>{hiroe}@<fn>{1flow}。
// Iwara.tv
// LesbianPornVideos
// きららファンタジア
// 東進ハイスクール在宅受講
//footnote[hiroe][特定の住民だけリソースが偏っていないか？というのはもっともですが、TCPから見ると均等です。部屋単位で均等するのはネットワーク管理者が対処すべきです。]
//footnote[1flow][専用線などで他のフローを排除して1つのフローだけを使うとリンクを100%使える……と良いのですが、現実にはそうならないようです。そのため、あえて複数のフローを作成して合計で100%に近づけるという方法を取ることがあります。ただ、これは専用線だけの話で、インターネットで@<bou>{発明}すると、他のフローの帯域を奪うので注意です。]
...
同様のことを行うアルゴリズムは多数ありますが、たとえばPCC@<fn>{dong15}@<fn>{allegro}ではCPU使用率が20%前後に達すると指摘されています@<fn>{leong17}。
CUBICやBBRは1%未満で、ワイヤレートとともに送信レートが上がることを考えると@<fn>{kernel}、リソースが足りるのかという懸念はあります。
//footnote[dong15][M.Dong, et al.，``PCC: Re-architecting Congestion Control for Consistent High Performance''，12th USENIX Symposium on Networked Systems Design and Implementation (NSDI 15)，2015．]
...
//footnote[dong18][M.Dong, et al.，``PCC Vivace: Online-Learning Congestion Control''，15th USENIX Symposium on Networked Systems Design and Implementation (NSDI 18)，2018．]
//footnote[leong17][WLeong, et al.，``TCP Congestion Control Beyond Bandwidth-Delay Product for Mobile Cellular Networks''，ACM CoNEXT，2017．]
//footnote[kernel][上げ過ぎたら、今後はカーネルのTCP/IPプロトコルスタックで処理しきれるかのほうが心配になりますが。]

PCCが特殊なだけかも知れませんが、今後の輻輳制御アルゴリズムはレイテンシやスループット、公平性などネットワークに関するパフォーマンスはもちろん、ホストの計算リソースを消費しないようなことも考慮されていくのかも知れません。
//chat[たまて]{
Copaはスループット・レイテンシ・公平性に優れている、ただ計算機のリソースを消費しないか心配ということですねー。
//}

=== DCTCP
いままではインターネットで使うような「汎用アルゴリズム」の話でしたが、データセンター内の通信で使う特定用途のアルゴリズムとして@<b>{DCTCP}@<fn>{alizadeh10}について述べます。
//footnote[alizadeh10][M.Alizadeh, et al.，``DCTCP: Efficient Packet Transport for the Commoditized Data Center''，ACM SIGCOMM，2010．]
...
特にインターネットではECNが使えるか否かなど分かるわけがなく、あまりあてにできません@<fn>{ecn}。
しかし、ここはデータセンターの中なので使いやすい状況にあり、ECNを輻輳回避に利用できます。
//footnote[ecn][最近はインターネットでもなるべくECNを使えるようにしようという動きがあるようです。]

このECNをそのまま使う、たとえばECNを受け取ったら輻輳とみなしてcwndを下げる……のでしょうか？
それだと、パケロスで輻輳を検知する場合と大して変わらないため、DCTCPではECNを検出したパケットの割合を考慮してcwndを決定します。
パケロスという1bitの情報量に対し、ECNの@<bou>{割合}を使うことで輻輳検知の情報量を増やしたわけですね。
これだけの変更でも、NewRenoに適用するとパフォーマンスが向上するらしく、効果はあるようです。
...
//footnote[soku][ECNを検知すると即座に輻輳と判断して、cwndを落とす挙動など。]
...
//footnote[mittal15][R.Mittal, et al.，``TIMELY: RTT-based Congestion Control for the Datacenter''，ACM SIGCOMM，2015．]
...
論文ではまさにClosネットワークで評価したようなので、問題ないのかも知れません。
確かに5-tupleで振り分けていたらうまくいきそうですが、ラウンドロビンにしていたらアウトじゃないですか？
...
//footnote[clos][そう思いはじめたら、既存の輻輳制御アルゴリズムとClosネットワークの相性が心配になってきました……。でも輻輳制御は各フロー（ソケット）ごとに行われることを考えたら、ECMPで5-tupleによって振り分ける場合は特に考えなくて良い気もします。ラウンドロビンにすればリンク使用率は上がると思いますが、それによる悪影響のほうが大きそうです。]
//chat[花名]{
DCTCPはECNを使って輻輳検知の情報量を増やす、TIMELYはRTTで情報量を増やすんだね。
//}

=== HPCC
次は@<b>{HPCC}@<fn>{li19}について述べます。
汎用アルゴリズムが「公道」の話、DCTCPやTIMELYが「私道」の話とすると、ここでは「専用軌道」@<fn>{guideway}の話です。
//footnote[li19][Y.Li, et al.，``HPCC: High Precision Congestion Control''，ACM SIGCOMM，2015．]
//footnote[guideway][鉄道ほどではなく「ガイドウェイバス」のことを思い浮かべながら書きました。RoCEなので。Infinibandは鉄道で良い気がします。]

というのも、HPCCでは@<ruby>{RoCE,ロッキー} (RDMA over Converged Ethernet) v2で利用することを想定しています。
RoCE v2は、RDMA (Remote Direct Memory Access)のデータ@<fn>{ib}をEthernetフレーム・IPパケット・UDPデータグラムでカプセル化する仕組みです。
...
//footnote[ib][InfiniBandのデータです。]
//footnote[driver][NICを制御するためにドライバは触りますが、データには触れません。]

RoCE v2で用いられる輻輳制御アルゴリズムには、@<b>{DCQCN}@<fn>{zhu15}があります。
...
//footnote[zhu15][Y.Zhu, et al.，``Congestion Control for Large-Scale RDMA Deployments''，ACM SIGCOMM，2015．]
...
@<img>{hpcc}のように、あるパケットがスイッチを通った際にスイッチがパケット内にINTを書いてくれるので、HPCCではこれをNICから利用します。
...
//footnote[p4][データプレーンのプログラミング言語です。この場合、最終的にはスイッチASICのAPIを叩くことになると思います。]
//footnote[silicom][スライドを見るとSilicom社の「fb2XGhh@V7」という製品に見えます。SFP+ 10GEx2でXilinx Virtex7 VX690TというFPGAが載るようです。]
//chat[たまて]{
...
//chat[花名]{
それじゃ、まとめをするね。
本稿ではレイヤ4輻輳制御の概要を述べ、具体的なアルゴリズムとしてReno、CUBIC、BBR、Copa、DCTCP、HPCCを紹介しました。
//}
//rchat[たまて]{
...
図～がぼくがかんがえたさいきょうの通信環境ですが、Raspberry PiをEthernetアダプタとしてiPadからの通信を吸い込み、自宅へProxy、そこから決済センターへ繋ぎます。
...
これマジ？なんのために図～を作ったのでしょうか？
Wi-Fiならこの構成を実現できますが、コミケ会場でWi-Fiを使うほうが不安定になりそうなのでボツです。
...
そのため、図～のようにProxyサーバを用意し、せめて少しの区間でも任意の輻輳制御アルゴリズムを適用させることを考えました。
...
//footnote[ymobile][手持ちの回線で、送信側ホストの輻輳制御アルゴリズムを変えながらスループット測定もやってみましたが、ひどい結果になったので割愛します。]
//footnote[taiho][何も考えずMacbookにRTL-SDRを繋ぎ、トートバッグに忍ばせて徘徊していましたが完全に不審者なので、UMPCやRaspberry Piが必要だったと思います。]

調査した結果は、@<img>{cell}のはずですが……大部分を黒塗りにしました。
...
もっとも、SIMなしの端末でETWS（緊急速報）を受信しても傍受、それをツイートしたら内容を漏らすことに該当しそうで、まず検挙されるわけがないとは思います。
//image[cell][LTE基地局の探索結果][scale=0.8]

しかし、実をいうと今回の測定では特筆すべきところがなく、あえてリスクを冒してまで載せるようなデータではありません@<fn>{omoshiroi}。
...
//footnote[omoshiroi][ある場所で検証した際は、特殊な事業者のMNC (Mobile Network Code)が見えて面白かったです。どえらい結果が出たぁあああああ！！！]
