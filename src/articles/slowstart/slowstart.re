...
ここでいう@<bou>{周り}には、自分が生み出した他のフローも含まれるため、結局は自分も損するだけです。
...
//image[reno][RenoやNewRenoにおけるcwndの変化]
...
PCCが特殊なだけかも知れませんが、今後の輻輳制御アルゴリズムの評価を行う際はレイテンシやスループット、公平性などネットワークに関するパフォーマンスはもちろん、ホストの計算リソースについても確認すべきなのかも知れません。
//chat[たまて]{
...
確かにフローに固定の経路、パケットごとに5-tupleからキーを求めて経路を決めていたらうまくいきそうです。
もし、リンク使用率を上げる目的でパケットごとのラウンドロビンをやろうとしても、輻輳制御アルゴリズムに対する影響を考える前に、パケットリオーダリングの影響でパフォーマンスが落ちることが懸念されるので、やらないでしょう。
...
//footnote[clos][そう思いはじめたら、既存の輻輳制御アルゴリズムとClosネットワークの相性が心配になってきました……。でも輻輳制御は各フロー（ソケット）ごとに行われることを考えたら、5-tupleをキーにして分散させるなら何も考えなくて良い気もします。ラウンドロビンにすればリンク使用率は上がると思いますが、それによる悪影響のほうが大きそうです。]
//chat[花名]{
