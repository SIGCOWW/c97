...
ただ、ここではTCPヘッダの中の「ウィンドウサイズ」は、受信ウィンドウ（rwnd: Receive Window）のこととします。自分が受信できるバイト（オクテット）数を相手に通知するわけですね。
...
//image[cwnd][cwnd][scale=0.49]
//image[sake][飲酒量][scale=0.49]
//}
次は輻輳制御アルゴリズムの紹介です。
...
//image[bbr][cwndに対するレイテンシ転送レートの関係（BBRの資料より引用）。横軸はcwndとほぼ同等、縦軸は上図と下図でそれぞれ独立している。][scale=0.8]
...
//image[cell][LTE基地局の探索結果][scale=0.9]
